(function(){
  ...
  document.addEventListener('click',async e=>{
    ...
    const clearCart=e.target.closest('[data-cart-clear]'); if(clearCart){if(!window.confirm('Kosongkan keranjang? Semua item akan dihapus.'))return;clearCart.disabled=true;try{await api.request('/api/cart',{method:'DELETE'});toast.success('Keranjang dikosongkan.');location.reload()}catch(err){toast.error(err.message)}finally{clearCart.disabled=false}}
  ...
  <div class="product-modal-actions">
    ...
    const dup=e.target.closest('[data-owner-duplicate]');if(dup){if(!window.confirm('Duplikat produk ini?'))return;try{await ownerAction('/api/owner/products/'+dup.dataset.ownerDuplicate+'/duplicate',{},'POST','Produk diduplikasi.');location.reload()}catch(err){toast.error(err.message)}}
