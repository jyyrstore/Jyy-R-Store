...
function windowSpec(ms){
  const seconds=Math.max(1,Math.round(Number(ms||60000)/1000));
  return `${seconds} s`;
}
...
function sharedOrLocalRateLimit(kind){
  ...
  const local=rateLimit({
    ...
    max:kind==='sensitive'?env.SENSITIVE_RATE_LIMIT_MAX:env.RATE_LIMIT_MAX,
  ...
  return async function sharedLimiter(req,res,next){
    ...
    try{
      const limiter=await getSharedLimiter(kind,env.RATE_LIMIT_WINDOW_MS,kind==='sensitive'?env.SENSITIVE_RATE_LIMIT_MAX:env.RATE_LIMIT_MAX);
      if(!limiter) return local(req,res,next);
      const identifier=`ip:${req.ip}`;
      const result=await limiter.limit(identifier);
      res.setHeader('x-ratelimit-limit',String(result.limit));
      res.setHeader('x-ratelimit-remaining',String(result.remaining));
      if(result.reset) res.setHeader('x-ratelimit-reset',String(Math.ceil(Number(result.reset)/1000)));
      if(!result.success){
        return res.status(429).json({
          ...
          error:{
            ...
            retryAfter:Math.max(1,Math.ceil((Number(result.reset)-Date.now())/1000))
          }
        });
      }
      return next();
    }catch(error){
      log('warn','distributed_rate_limit_unavailable',{
        kind,
...
function generalRateLimit(){return sharedOrLocalRateLimit('general');}
function sensitiveRateLimit(){return sharedOrLocalRateLimit('sensitive');}
