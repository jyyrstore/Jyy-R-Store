...
async function checkout(req,res,next){ try { const paymentMethod=String(req.query.payment||'').toUpperCase()==='GATEWAY'?'GATEWAY':'BALANCE';res.render('app',base(req,{view:'pages/checkout',preview:await orderService.preview(req.user.id),selectedPaymentMethod:paymentMethod}));}catch(e){next(e)} }
