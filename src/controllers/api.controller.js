...
const ownerController={
 ...
 products:async(req,res)=>ok(res,await require('../repositories/products.repository').list({search:req.query.search,sort:req.query.sort||'newest',status:req.query.status||'PUBLISHED',limit:req.query.limit||100})),
 productCreate:async(req,res)=>{const p=await product.create(req.body);await require('../services/audit.service').record(req,{action:'CREATE_PRODUCT',entityType:'product',entityId:p.id,metadata:{name:p.name}});return ok(res,p,201)},
 ...
 productPublish:async(req,res)=>{const p=await product.update(req.params.id,{status:'PUBLISHED'});await require('../services/audit.service').record(req,{action:'PUBLISH_PRODUCT',entityType:'product',entityId:p.id});return ok(res,p)},
 productContent:async(req,res)=>{const p=await require('../repositories/products.repository').addContent({...req.body,product_id:req.params.id});return ok(res,p,201)},
